...
__global__ void evolvi(curandState* stato,float*x, float * eta,float * forza){float w;int id=threadIdx.x+ blockIdx.x*blockDim.x;	
							w=curand_normal(&stato[id])*sqrt(2.);	
							//w=0;
							eta[id]=eta[id]-(1/tau)*eta[id]*dt+sqrt(D)*w*sqrt(dt)/tau; 
							x[id]=x[id]+eta[id]*dt;
							float delta;forza[id]=0;
							if (x[id]+raggio>half_size){delta=x[id]+raggio-half_size;x[id]=half_size-raggio;forza[id]=delta/(dt*mu);}
