...
#include <sys/types.h>
#include <unistd.h>
#define vel 0.1 // costante nel processo di O-U
#define prob 0.3
#define dt 0.1
...
//const int durata=prob*size*size/(vel*vel);
...
__global__ void inizializza(curandState* stato,float *x,float *eta){int id=threadIdx.x+ blockIdx.x*blockDim.x;
					x[id]=curand_uniform(&stato[id])*size-half_size;eta[id]=vel;}

__global__ void evolvi(curandState* stato,float*x, float * eta,float * forza){float w,incremento;int id=threadIdx.x+ blockIdx.x*blockDim.x;	
							w=curand_uniform(&stato[id]);
							if (w<prob)	incremento=1.;
							else		incremento=-1.;
							//w=0;
							eta[id]=eta[id]*incremento; 
							x[id]=x[id]+eta[id]*dt;
							float delta;forza[id]=0;
							if (x[id]+raggio>half_size){delta=x[id]+raggio-half_size;
											x[id]=half_size-raggio;forza[id]=delta/(dt*mu);}
						else if (x[id]-raggio<-half_size) {delta=x[id]-raggio +half_size;x[id]=-half_size+raggio;forza[id]=delta/(dt*mu);} 
							
										}
void 			stampa(float* x,int m)	{int i;for(i=0;i<m;i++) printf("%1.4f \n",x[i]);}
void		stampa_file(float*x, int m){FILE*f;char indirizzo [50];int i;int N=blocks*threads;
...
float *traettoria; float * storage;float* forza; float* dev_forza;float* velocit; 
traettoria=(float*)malloc(durata*sizeof(float));
velocit=(float*)malloc(durata*sizeof(float));
...
inizializza<<<blocks,threads>>>(stato,sistema.x,sistema.eta);
int i=0;
for(t=0;t<durata;t++){	
			evolvi<<<blocks,threads>>>(stato,sistema.x,sistema.eta,dev_forza);
