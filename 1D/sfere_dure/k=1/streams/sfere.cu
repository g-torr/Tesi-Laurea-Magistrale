...
#define k 1.	//convessitù parabola potenziale armonico
#define D 0.1
const float size= 10.;
...
const float durata=size*size/(2.*D)*100.;
const float tsalva= durata/500.;
...
__global__ void inizializza(double *x,double *eta){int id=threadIdx.x+ blockIdx.x*blockDim.x;
					x[id]=half_size-(1./half_size);eta[id]=0.5;}
...
__global__ void evolvi(curandState* stato,double*x, double * eta,double* forza,double dt){double w;int id=threadIdx.x+ blockIdx.x*blockDim.x;	
	...
	x[id] = x[id] +forza[id]*dt + eta[id]*dt;// processo dinamico 
        ...
        //	 if (x[id]>=half_size) x[id] = x[id]-(size*floor(x[id]/half_size));
	//				else if(x[id]< -half_size) x[id] = x[id]-(size*ceil(x[id]/half_size));		
...
cudaStream_t stream0,stream1,stream2,stream3;// creo 4 stream
cudaStreamCreate(&stream0);cudaStreamCreate(&stream1);cudaStreamCreate(&stream2);cudaStreamCreate(&stream3);//****creo gli stream
configurazione sistema_0,sistema_1,sistema_2,sistema_3;//definisco 4  variabili stato del sistema sul device 
double* dev_forza_0;double* dev_forza_1;double* dev_forza_2;double* dev_forza_3;
   int N=blocks*threads;int t; // x è il sistema  dinamico, n è il rumore
  cudaEvent_t start,stop; 
  double *traettoria; double * storage; double* forza;  
  traettoria=(double*)malloc(durata*sizeof(double));
  cudaEventCreate(&start);
  cudaEventCreate(&stop);
  cudaEventRecord(start,0);
cudaHostAlloc((double**)&storage,N*sizeof(double),cudaHostAllocDefault);
cudaHostAlloc((double**)&forza,N*sizeof(double),cudaHostAllocDefault);

cudaMalloc((double**)&sistema_0.x,(N/4)*sizeof(double));
cudaMalloc((double**)&sistema_0.eta,(N/4)*sizeof(double));
cudaMalloc((double**)&dev_forza_0,(N/4)*sizeof(double));

cudaMalloc((double**)&sistema_1.x,(N/4)*sizeof(double));
cudaMalloc((double**)&sistema_1.eta,(N/4)*sizeof(double));
cudaMalloc((double**)&dev_forza_1,(N/4)*sizeof(double));

cudaMalloc((double**)&sistema_2.x,(N/4)*sizeof(double));
cudaMalloc((double**)&sistema_2.eta,(N/4)*sizeof(double));
cudaMalloc((double**)&dev_forza_2,(N/4)*sizeof(double));


cudaMalloc((double**)&sistema_3.x,(N/4)*sizeof(double));
cudaMalloc((double**)&sistema_3.eta,(N/4)*sizeof(double));
cudaMalloc((double**)&dev_forza_3,(N/4)*sizeof(double));

curandState * stato;
cudaMalloc((void**)&stato,N*sizeof(curandState));
setup_random_kernel<<<blocks,threads>>>(stato);

double dt;int j;dt=.001630;
	int numero_passi,passi_salvataggio,passi_termalizzazione;

for(j=0;j<3;j++){
	dt=dt/2.;
  printf("%f\n",dt);
 inizializza<<<blocks/4,threads,0,stream0>>>(sistema_0.x,sistema_0.eta);
 inizializza<<<blocks/4,threads,0,stream1>>>(sistema_1.x,sistema_1.eta);
 inizializza<<<blocks/4,threads,0,stream2>>>(sistema_2.x,sistema_2.eta);
 inizializza<<<blocks/4,threads,0,stream3>>>(sistema_3.x,sistema_3.eta);  
...
numero_passi=(int)(durata/dt);
  ...
  int i=0;
   ...
   for(t=0;t<numero_passi;t++){
    evolvi<<<blocks/4,threads,0,stream0>>>(stato,sistema_0.x,sistema_0.eta,dev_forza_0,dt);
    evolvi<<<blocks/4,threads,0,stream1>>>(stato,sistema_1.x,sistema_1.eta,dev_forza_1,dt);
    evolvi<<<blocks/4,threads,0,stream2>>>(stato,sistema_2.x,sistema_2.eta,dev_forza_2,dt);
    evolvi<<<blocks/4,threads,0,stream3>>>(stato,sistema_3.x,sistema_3.eta,dev_forza_3,dt);
//			if(t>=termalizzazione)cudaMemcpy(&traettoria[t-termalizzazione],sistema.x,sizeof(float),cudaMemcpyDeviceToHost);}
    if((t% passi_salvataggio==0)&&(t>passi_termalizzazione)){
      ...
      cudaMemcpyAsync(forza,dev_forza_0,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream0);
      cudaMemcpyAsync(forza+(N/4),dev_forza_1,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream0);
      cudaMemcpyAsync(forza+(N/2),dev_forza_2,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream0);
      cudaMemcpyAsync(forza+(3*N/4),dev_forza_3,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream0);
	cudaDeviceSynchronize(); 
     stampa_file(forza,i,dt);i++;
    }
  }

  cudaMemcpyAsync(storage,sistema_0.x,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream0);
 cudaMemcpyAsync(storage+(N/4),sistema_1.x,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream1);
 cudaMemcpyAsync(storage+(N/2),sistema_2.x,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream2);
 cudaMemcpyAsync(storage+(3*N/4),sistema_3.x,(N/4)*sizeof(double),cudaMemcpyDeviceToHost,stream3); 
	cudaDeviceSynchronize();
  stampa(storage,dt);
 }


cudaFreeHost(storage);
cudaFreeHost(forza);
cudaFree(sistema_0.x);
cudaFree(sistema_0.eta);
cudaFree(dev_forza_0);

cudaFree(sistema_1.x);
cudaFree(sistema_1.eta);
cudaFree(dev_forza_1);

cudaFree(sistema_2.x);
cudaFree(sistema_2.eta);
cudaFree(dev_forza_2);

cudaFree(sistema_3.x);
cudaFree(sistema_3.eta);
cudaFree(dev_forza_3);
