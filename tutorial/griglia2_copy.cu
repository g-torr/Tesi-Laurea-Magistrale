...
void	time_step(dati* T,float** storage)	{bool flag=true; int n;
				
				dim3 blocks(DIM/threadsperblock,DIM/threadsperblock);
				dim3 threads (threadsperblock,threadsperblock);
				for(n=0;n<durata;n++){
				float * in; float *out;
				if (flag){in=T->old;		 out=T->upd;printf("si"); }
				else	{in=T->upd;		 out=T->old;printf("no");}
				apply_constrains<<<blocks,threads>>>(in);
				execute<<<blocks,threads>>>(out,flag);
					flag=!flag;			
				
				if (flag){cudaMemcpy(storage[n],T->old,DIM*DIM*sizeof(float),cudaMemcpyDeviceToHost);printf(" e ancora no \n");}
				else	{cudaMemcpy(storage[n],T->upd,DIM*DIM*sizeof(float),cudaMemcpyDeviceToHost);printf(" e ancora si \n");}							}			
...
for(n=0;n<durata;n++)storage[n]=(float*)malloc(DIM*DIM*sizeof(float));
bool  flag=true;

time_step(&T_dev,storage);
