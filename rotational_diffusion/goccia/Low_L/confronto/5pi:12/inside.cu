...
const float size=(float)4.;
const float half_size=size/(float)2.;
#define v (float)0.5 // velocità 
#define D (float)6. //diffusività rotazionale
#define dt (float)0.1
#define mobility (float)1
#define N 51
const float durata=10*size*size/(v*v/D);
...
void	stampa(configurazione stato, int i){int id;FILE*f;FILE*g;char indirizzo_posizione[50];char indirizzo_forza[50];char indirizzo_velocity[50];
...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
			...
			float th;
			int id=threadIdx.x+blockIdx.x*blockDim.x; 	
			th=stato.theta[id];	r=stato.r[id];
			th=th+curand_normal(&gen_random[id])*costanti[0];   		
			r.x=r.x+v*dt*cos(th);
			r.y=r.y+v*dt*sin(th);	
			...
			stato.theta[id]=th;		stato.r[id]=r;		
				/*	if (in){
										f=r;
										r=segmento_vicino(r);
...
cudaMalloc((float**)&dev_stato.theta,particelle*sizeof(float));
...
stato.theta=(float*)malloc(particelle*sizeof(float));
...
for(t=0;t<numero_passi;t++){
		...
		//	printf("numero di pallette che entrano %d \n", count);
			...
			correct<<<(count+1023)/1024,1024>>>(dev_stato,generatori_random,dev_ids,count);					
			   ...
			   if((t% passi_salvataggio==0)&&(t>0)){printf("siamo a %d/15 \n",i);
			...
			cudaMemcpy(stato.theta,dev_stato.theta,particelle*sizeof(float),cudaMemcpyDeviceToHost);
				stampa(stato,i);i++;
