...
#define tau 0.02 // costante nel processo di O-U
#define D 0.5
#define dt 0.001
#define mobility 1
const int durata=(int)size*size*10/D;
const float tsalva= durata/10.;
const float raggio=0.5; //ricordati che deve essere minore di half_size
...
//for(t=0;t<10;t++){
	   ...
	   if((t% passi_salvataggio==0)&&(t>0)){printf("siamo a %d/9 \n",i);
			...
			cudaMemcpyAsync(stato.eta,dev_stato_0.eta,(particelle/4)*sizeof(point),cudaMemcpyDeviceToHost,stream0); 			
			cudaMemcpyAsync(stato.eta + (particelle/4),dev_stato_1.eta,(particelle/4)*sizeof(point),cudaMemcpyDeviceToHost, stream1);
			cudaMemcpyAsync(stato.eta + (particelle/2),dev_stato_2.eta,(particelle/4)*sizeof(point),cudaMemcpyDeviceToHost, stream2);
			cudaMemcpyAsync(stato.eta + (3*particelle/4),dev_stato_3.eta,(particelle/4)*sizeof(point),cudaMemcpyDeviceToHost, stream3);	
