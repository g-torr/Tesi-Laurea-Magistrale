...
def	media_temporale(start,finish,r1=1,r2=1,D=1.,tau=0.000625,nb=150):
	figure(0)
...
def	probability_in_curvature(r1,r2,D,tau,nof=9):
	...
	'''	
def bulk_probability(start,stop):
	R=1.
	print("Sto calcolando la density  of bulk solo al centro ad una particella ")
	area=pi*R**2
	...
	r0=R-2*L
	r=sqrt(x**2+y**2)
	number_bulk=count_nonzero(r<r0)/float(len(x))#probability of bulk at 1 particle
	rho=number_bulk/(pi*r0**2) #densita' di bulk
	print(" density of bulk normalized at 1 particle= "+str(rho))
	return rho
...
def pressione(start,stop):
  ...
  p=np.array(p)
  p=p/(2*pi*R)
  var_p=std(F)/(stop-start+1)/(2*pi*R)
  r0=R-0.5*L
  r=sqrt(x**2+y**2)
  number_bulk=count_nonzero(r<r0)/(stop-start+1)#particelle sul bulk
  rho=number_bulk/(pi*r0**2) #densita' di bulk
  print(" density of bulk = "+str(rho/len(temp_x)))
  p=p/rho #normalizzo rispetto alla densita' di bulk
  plot(arange(start,stop+1),p,"o")
  xlabel("time")
  ylabel("normalized pressure")
  savefig("pressione.pdf")
  np.save("pressione.npy",array([R,mean(p),std(p)/sqrt(stop-start+1)]))
