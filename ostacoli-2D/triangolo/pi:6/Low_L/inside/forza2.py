...
def forza2(start,stop):
	...
	tx=2*rand(1e6)-1
	ty=2*rand(1e6)-1
	area=len(filter(filtering_function,array([tx,ty]).T))/float(len(tx))*4.
	rho=count_nonzero(array(filter(filtering_function,array([x,y]).T)).T)/area #densita' di bulk
