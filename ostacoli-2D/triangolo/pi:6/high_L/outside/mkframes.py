...
def media(start,finish,nob=100):
	...
	for i in arange(start+1,finish+1):
		...
		hvx_sum +=hvx
		hvy_sum +=hvy
	hvx_sum=hvx_sum/float(finish-start+1)
	hvy_sum=hvy_sum/float(finish-start+1)
	hJx_sum=hJx_sum/float(finish-start+1)
	hJy_sum=hJy_sum/float(finish-start+1)
	V=sqrt(hvx_sum**2+hvy_sum**2)
	J=sqrt(hJx_sum**2+hJy_sum**2)
	quiver(hx,hy,hJx_sum,hJy_sum,J/v)
	colorbar(label="$J/v$")
	return hx,hy,hvx_sum,hvy_sum,hJx_sum,hJy_sum
