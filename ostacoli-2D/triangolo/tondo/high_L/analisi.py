...
def	campo_forze(nf):
	...
	f=hfx**2+hfy**2
	print(len(bx),len(by))
	quiver(bx[:-1]+xstep,by[:-1]+ystep,hfx/nf,hfy/nf,f,scale=10)
...
def	campo_velocity(nf=8):
	x,y=transpose(loadtxt("./posizione/dati_"+str(1)))
	vx,vy=transpose(loadtxt("./velocity/velocity_"+str(1)))
	#modulo=sqrt(fx*fx+fy*fy)
	xmn = min(x)
        xmx = max(x)
        ymn = min(y)
	...
	figure()
	title("current field")
	quiver(bx[:-1]+xstep,by[:-1]+ystep,Jx/float(nf),Jy/float(nf),v,scale=1000)
#        quiver(by[:-1],bx[:-1],hfy,hfx,f,scale=20)
