...
#define tau (float)2. // costante nel processo di O-U
#define D (float)20.
#define dt (float)0.0001
#define mobility (float)1
#define N 4
const float durata=size*size*20/D;
...
__global__ void correct(configurazione stato,curandState* gen_random,int * ids,int count){
				...
				stato.eta[id].x=v.x-delta.x/dt;	stato.eta[id].y=v.y-delta.y/dt;		stato.r[id]=r;		
