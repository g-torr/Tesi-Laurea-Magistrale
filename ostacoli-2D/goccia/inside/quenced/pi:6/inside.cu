...
#define D (float)0.1
#define dt (float)0.005
#define mobility (float)1
#define N 51
const int durata=size*4500/(sqrt(D/tau));
...
__global__ void	inizializza(configurazione stato,curandState* gen_random){point temp;
							...
							do {
							temp.x=-size*(1/6.)+size*(2./3.)*curand_uniform(&gen_random[id]);
							temp.y=-size*(1/6.)+size*(2./3.)*curand_uniform(&gen_random[id]);}
							
								while(!is_in(temp)); 		 	     							stato.r[id].x=temp.x;
						stato.r[id].y=temp.y;
						stato.eta[id].x=sqrt(D/tau)*curand_normal(&gen_random[id]);	
						stato.eta[id].y=sqrt(D/tau)*curand_normal(&gen_random[id]);
...
printf("sarebbe opportuno che aggiusti la size per far termalizzare bene il sistema\n");
...
printf("finito di  inizializzare le particelle\n");
i=0;int count;
int*  ids;int*  dev_ids;
