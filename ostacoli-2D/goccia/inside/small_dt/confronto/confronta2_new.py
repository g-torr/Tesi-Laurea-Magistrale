def confronta():
	...
	b,h=np.load("./pi:12/F_old.npy")
	theta=pi/12
	cond=(b>L+sin(theta))&(b<max(b)-2*L*cos(theta))
	'''cond1=b>max(b)-2*L
	p=polyfit(b[cond], h[cond] ,1)'''
	angolo+=[theta]
	value=append(value,max(h)-diff(b)[0]*mean(h[cond])*cos(theta))
	plot(b,h)	

	b,h=np.load("./pi:6/F.npy")
	theta=pi/6
	cond=(b>L*cos(theta)+sin(theta))&(b<max(b)-2*L*cos(theta))
	'''cond1=b>max(b)-2*L
	p=polyfit(b[cond], h[cond] ,1)'''
	angolo+=[theta]
	value=append(value,max(h)-diff(b)[0]*mean(h[cond])*cos(theta))
	...
	theta=5*pi/24
	cond=(b>L*cos(theta)+sin(theta))&(b<max(b)-L*cos(theta))
	'''cond1=b>max(b)-2*L
	p=polyfit(b[cond], h[cond] ,1)'''
	angolo+=[theta]
	value=append(value,max(h)-diff(b)[0]*mean(h[cond])*cos(theta))
	...
	legend(["$\\frac{\pi}{12}$","$\\frac{\pi}{6}$","$ \\frac{5\pi}{24}$","$\\frac{\pi}{4}$","$\\frac{7pi}{24}$","$\\frac{\pi}{3}$","$\\frac{3pi}{8}$","$\\frac{5}{12}$"],"best")
	figure()	
