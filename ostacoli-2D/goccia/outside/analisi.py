...
def	media_temporale(nf=9,nb=150):
	...
	figure(1)
	plot(by,hy,'-k',lw=2)
	np.save("fy_histogram.npy",array([by,hy]))
...
def	modulo_forza(r1,r2,D=0.1,tau=0.06):
	fx,fy=transpose(loadtxt("./forza/forza_0"))
	modulo =sqrt(fx**2+fy**2)
	for i in arange(1,8):
		fx,fy=transpose(loadtxt("./forza/forza_"+str(i)))
...
def	campo_velocity(nf=8):
	...
	print(len(bx),len(by))
	#print(hvy)
	quiver(bx[:-1]+xstep,by[:-1]+ystep,hvx/float(nf),hvy/float(nf),v,scale=10)
#        quiver(by[:-1],bx[:-1],hfy,hfx,f,scale=20)
