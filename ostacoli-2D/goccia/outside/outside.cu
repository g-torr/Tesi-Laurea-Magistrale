...
const float size_y=(float)16;
const float size_x=(float)12;
const float half_size_y=size_y/(float)2.;
const float half_size_x=size_x/(float)2.;
#define tau (float)5. // costante nel processo di O-U
#define D (float)5.
#define dt (float)0.001
#define mobility (float)1
#define N 350
const float durata=size_y*sqrt(tau/D)*20;
...
__global__ void	inizializza(configurazione stato,curandState* gen_random){point temp;
							...
							do {
							temp.x=-half_size_x+size_x*curand_uniform(&gen_random[id]);
							temp.y=-half_size_y+size_y*curand_uniform(&gen_random[id]);}
...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
			...
			//if((r.x>ottimizza[0].x)&&(r.x<ottimizza[1].x)&&(r.y<ottimizza[2].x)&&(r.y>ottimizza[0].y)){		
					...
					if(in) stato.inside[id]=true;
						...
						if(r.x>half_size_x)	r.x=r.x-size_x;
						else if (r.x<-half_size_x) r.x=r.x+size_x;
						if	(r.y>half_size_y)	r.y=r.y-size_y;
						else if (r.y<-half_size_y)	r.y=r.y+size_y;}
...
__global__ void correct(configurazione stato,curandState* gen_random,int * ids,int count){
				...
				delta.x=delta.x-r.x;
				delta.y=delta.y-r.y;
				stato.forza[id].x=delta.x/(dt*mobility);
				stato.forza[id].y=delta.y/(dt*mobility); 
						stato.r[id]=r;		
...
for(t=0;t<numero_passi;t++){
		...
		//	printf("numero di pallette che entrano %d \n", count);
			...
			correct<<<(count+1023)/1024,1024>>>(dev_stato,generatori_random,dev_ids,count);					
			   ...
			   if((t% passi_salvataggio==0)&&(t>0)){printf("siamo a %d/19 \n",i);
