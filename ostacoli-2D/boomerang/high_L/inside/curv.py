...
def boomerang(theta=pi/20):#out[0] ed out[1] sono rispettivamente ascisse e ordinate della mia curva
 ...
 for i in arange (0,len(out[0])):  
	...
	normal[0]=append(normal[0],d1[1][i]/modulo[i])
	normal[1]=append(normal[1],-d1[0][i]/modulo[i])
	somma[0]=append(somma[0],modulo[i]*normal[0][i]*k[i])*dt
	somma[1]=append(somma[1],modulo[i]*normal[1][i]*k[i])*dt

 D=1.
 tau=1.	
 #return somma
 print( " i risultati vanno moltiplicati per sqrt(D/tau")
 print("F_x risultante"+str(sum(somma[0]))+"  F_y risultante"+str(sum(somma[1])))
 quiver(out[0],out[1],normal[0],normal[1])
 print("arc lenght="+str(arc_lenght))
