...
def	media_temporale(start,finish=9,nb=150):
	figure(0)
	...
	plot(by,hy,'-k',lw=2)
	modulo_forza(start,finish)
	figure(3)
	xlabel("t")
	ylabel("Fy")
	savefig("Fy_thermalization.pdf")
	figure(4)
	xlabel("t")
	ylabel("Fx")
	savefig("Fx_thermalization.pdf")
	'''fx = (b1[1:]+b1[:-1])/2.
	figure(1)
...
def	forza_netta(first,last):
        fx,fy=transpose(loadtxt("./forza/forza_"+str(first)))
        n=len(fx[(fx!=0)|(fy!=0)])
	somma_x=sum(fx)
	somma_y=sum(fy)
	N=len(fx)
        for i in arange(first+1,last):
                tempx,tempy=transpose(loadtxt("./forza/forza_"+str(i)))
		somma_x+=sum(tempx)
		somma_y+=sum(tempy)
		n+=len(tempx[(tempx!=0)|(tempy!=0)])
		N+=len(tempx)
...
def	probability_in_curvature(nof=9):
	x=[]
	y=[]
	fx=[]
	fy=[]
