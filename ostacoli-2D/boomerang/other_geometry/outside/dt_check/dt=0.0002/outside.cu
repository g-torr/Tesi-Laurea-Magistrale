...
const float size=(float)13.;
const float half_size=size/(float)2.;
#define tau (float).06 // costante nel processo di O-U
#define D (float).1
#define dt (float)0.0002
#define mobility (float)1
#define N 100

const float durata=size*sqrt(tau/D)*20;

const int blocks=1024;
const int threads=1024;
const int particelle=blocks*threads;
int numero_passi=(int)(durata/dt);
int passi_salvataggio =(int)(durata/(20.*dt));
...
__global__ void	inizializza(configurazione stato,curandState* gen_random,int stream){point temp;
							int id=threadIdx.x+blockIdx.x*blockDim.x;	
						
							do {
							temp.x=-half_size+size*curand_uniform(&gen_random[id+stream*particelle/2]);
							temp.y=-half_size+size*curand_uniform(&gen_random[id+stream*particelle/2]);}
...
__global__ void	evolvi(configurazione stato,curandState* gen_random,int stream){	
			point r,v,f;
			int id=threadIdx.x+blockIdx.x*blockDim.x; 	
			v=stato.eta[id];	r=stato.r[id];
			v.x=v.x-v.x*costanti[0]+curand_normal(&gen_random[id+stream*particelle/2])*costanti[1];   	
			v.y=v.y-v.y*costanti[0]+curand_normal(&gen_random[id+stream*particelle/2])*costanti[1]; 	
			r.x=r.x+v.x*dt;
			r.y=r.y+v.y*dt;	
...
__global__ void correct(configurazione stato,int * ids,int count){
				point r,f;
...
cudaStreamCreate(&stream0);cudaStreamCreate(&stream1);//****creo gli stream
configurazione dev_stato_0,dev_stato_1,stato;//alloco lo stato del sistema sul device e sull'host tramite cudaHostAlloc (pinned memory)

cudaMalloc((point**)&dev_stato_0.eta,(particelle/2)*sizeof(point));
cudaMalloc((point**)&dev_stato_0.r,(particelle/2)*sizeof(point));
cudaMalloc((point**)&dev_stato_0.forza,(particelle/2)*sizeof(point));
cudaMalloc((bool**)&dev_stato_0.inside,(particelle/2)*sizeof(bool));


cudaMalloc((point**)&dev_stato_1.eta,(particelle/2)*sizeof(point));
cudaMalloc((point**)&dev_stato_1.r,(particelle/2)*sizeof(point));
cudaMalloc((point**)&dev_stato_1.forza,(particelle/2)*sizeof(point));
cudaMalloc((bool**)&dev_stato_1.inside,(particelle/2)*sizeof(bool));
...
inizializza<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random,0);
inizializza<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random,1);
...
bool* inside_0;bool *inside_1;
cudaHostAlloc((bool**)&inside_0,(particelle/2)*sizeof(bool),cudaHostAllocDefault);
cudaHostAlloc((bool**)&inside_1,(particelle/2)*sizeof(bool),cudaHostAllocDefault);


i=0;int count0,count1;
int*  ids_0;int*  dev_ids_0;
int*  ids_1;int*  dev_ids_1;
cudaMalloc((int**)&dev_ids_0,(particelle/2)*sizeof(int));
cudaMalloc((int**)&dev_ids_1,(particelle/2)*sizeof(int));
cudaHostAlloc((int**)&ids_0,(particelle/2)*sizeof(int),cudaHostAllocDefault);
cudaHostAlloc((int**)&ids_1,(particelle/2)*sizeof(int),cudaHostAllocDefault);

for(t=0;t<numero_passi;t++){
			evolvi<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random,0);
			evolvi<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random,1);
			cudaMemcpyAsync(inside_0,dev_stato_0.inside,(particelle/2)*sizeof(bool),cudaMemcpyDeviceToHost,stream0);
			cudaMemcpyAsync(inside_1,dev_stato_1.inside,(particelle/2)*sizeof(bool),cudaMemcpyDeviceToHost,stream1); 					cudaDeviceSynchronize();	
			count0=0;	count1=0;
			for(int j=0;j<particelle/2;j++){//printf("%d \n",inside_0[j]);
					if(inside_0[j]==true){ ids_0[count0]=j;count0++;}
					if(inside_1[j]==true){ ids_1[count1]=j;count1++;}
							}
			//cudaDeviceSynchronize();	
		//	printf("numero di pallette che entrano %d \n", count);
			cudaMemcpyAsync(dev_ids_0,ids_0,(count0)*sizeof(int),cudaMemcpyHostToDevice,stream0);
			cudaMemcpyAsync(dev_ids_1,ids_1,(count1)*sizeof(int),cudaMemcpyHostToDevice,stream1);
			// printf("count0= %d\n",count0);
			//cudaDeviceSynchronize();
	  		
			correct<<<(count0+1023)/1024,1024,0,stream0>>>(dev_stato_0,dev_ids_0,count0);
			correct<<<(count1+1023)/1024,1024,0,stream1>>>(dev_stato_1,dev_ids_1,count1); 					
				cudaDeviceSynchronize();			
			   if((t% passi_salvataggio==0)&&(t>0)){printf("siamo a %d/9 \n",i);
			cudaMemcpyAsync(stato.r,dev_stato_0.r,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost,stream0);
			cudaMemcpyAsync(stato.r + (particelle/2),dev_stato_1.r,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost,stream1);
			cudaMemcpyAsync(stato.eta,dev_stato_0.eta,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost,stream0);
			cudaMemcpyAsync(stato.eta + (particelle/2),dev_stato_1.eta,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost,stream1); 
 
			cudaMemcpyAsync(stato.forza,dev_stato_0.forza,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost,stream0); 			
			cudaMemcpyAsync(stato.forza + (particelle/2),dev_stato_1.forza,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost, stream1);
			
	  		cudaDeviceSynchronize();				stampa(stato,i);i++;
