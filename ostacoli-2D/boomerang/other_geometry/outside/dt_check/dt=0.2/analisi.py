...
def	campo_velocity(nf=8):
	...
	xstep=(xmx-xmn)/100
	ystep=(ymax-ymn)/100
	bns=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)]
	...
	hx,bx,by = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=x)
	hy,bx,by = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=y)
	Jx =  hvx
	Jy =  hvy
	h = h.flatten()
	hx = hx.flatten()
	...
	for i in arange (1,nf):					
		...
		h_temp,bx_temp,by_temp = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)])
		hvx_temp,bx_temp,by_temp = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=vx)
		hvy_temp,bx_temp,by_temp = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=vy)
		hx_temp,bx_temp,by_temp = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=x)
		hy_temp,bx_temp,by_temp = histogram2d(x,y,bins=[arange(xmn,xmx,xstep),arange(ymn,ymax,ystep)],weights=y)
		Jx  +=hvx_temp
		Jy  +=hvy_temp
		h_temp = h_temp.flatten()
		hx_temp = hx_temp.flatten()
		hy_temp = hy_temp.flatten()
		hvx_temp = hvx_temp.flatten()
		hvy_temp = hvy_temp.flatten()
		Jx_temp =Jx_temp.flatten()
		Jy_temp =Jy_temp.flatten()
		hx_temp = compress(h_temp>0,hx_temp)/compress(h_temp>0,h_temp)
		hy_temp = compress(h_temp>0,hy_temp)/compress(h_temp>0,h_temp)
		hvx_temp = compress(h_temp>0,hvx_temp)
		hvy_temp = compress(h_temp>0,hvy_temp)
		hvx+=hvx_temp
		hvy += hvy_temp
		v=hvx**2+hvy**2
	...
	#print(hvy)
	title("vector field")
	quiver(by[:-1]+ystep,bx[:-1]+xstep,hvy/float(nf),hvx/float(nf),v,scale=5)
	colorbar()
	figure()
	title("current field")
	quiver(by[:-1]+ystep,bx[:-1]+xstep,Jy/float(len(x)*nf),Jx/float(len(x)*nf),v,scale=0.002)
#        quiver(by[:-1],bx[:-1],hfy,hfx,f,scale=20)
