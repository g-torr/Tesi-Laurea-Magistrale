...
def boomerang(theta=pi/20):#out[0] ed out[1] sono rispettivamente ascisse e ordinate della mia curva
 ...
 figure(3)
 if sign == -1:
	size=input("inserisci la size dell'oggetto")
 else:
	 size=0
 superficie=size**2+sign*superficie
 print("arc lenght="+str(arc_lenght)+" area= "+str(superficie))
 np.save("area.npy",[superficie,arc_lenght]) 
 ...
 pressure=abs(sum(modulo*dt*D*abs(sqrt(D*tau)*k+1)))/normalizzazione
 print(" pressure = "+str(pressure/float(arc_lenght)))
