...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
			...
			curandState localState = gen_random[id]; //piuttosto che andare a leggere nella global, le letture le faccio nella local memory ( che potrebbe essere la velocissima register)

			for(int t=0;t<passi_salvataggio;t++){
					v.x=v.x-v.x*costanti[0]+curand_normal(&localState)*costanti[1];   	
					v.y=v.y-v.y*costanti[0]+curand_normal(&localState)*costanti[1]; 	
					r.x=r.x+v.x*dt;
					r.y=r.y+v.y*dt;	
					f.x=0.f; f.y=0.f;
