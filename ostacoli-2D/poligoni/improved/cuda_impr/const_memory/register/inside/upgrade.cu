...
struct __align__(8) point{	float x;
		float y;};


struct __align__(16) configurazione{	point*	eta;
			point*	r;
			point*	forza;};
...
__device__ point segmento_vicino(point P){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
	int i=0;float t;float d; 
	 point closest;
	t = (float)((P.x-vertice[0].x)*(vertice[0+1].x-vertice[0].x)+(P.y-vertice[0].y)*(vertice[0+1].y-vertice[0].y))/
	...
	float d_temp;
 	for(i=1;i<N;i++){
    t = (float)((P.x-vertice[i].x)*(vertice[i+1].x-vertice[i].x)+(P.y-vertice[i].y)*(vertice[i+1].y-vertice[i].y))/
