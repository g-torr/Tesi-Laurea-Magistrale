...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
						...
						int id=threadIdx.x+blockIdx.x*blockDim.x; 
						curandState local_state=gen_random[id];	
				...
				for(int t=0;t<passi_salvataggio;t++){
					...
					//	v.y=v.y-(1/tau)*v.y*dt+sqrt(D)*curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau; 	
						v.x=v.x-v.x*costanti[0]+curand_normal(&local_state)*costanti[1];   	
						v.y=v.y-v.y*costanti[0]+curand_normal(&local_state)*costanti[1]; 	
						r.x=r.x+v.x*dt;
						r.y=r.y+v.y*dt;	
						f.x=0.f; f.y=0.f;
