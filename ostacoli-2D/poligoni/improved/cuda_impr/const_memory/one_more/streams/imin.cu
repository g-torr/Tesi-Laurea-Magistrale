...
const int numberofblocks=imin(32,(particelle+ threads-1)/threads);
...
for(t=0;t<numero_passi;t++){
			evolvi<<<numberofblocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random,0);
			evolvi<<<numberofblocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random,1);
		...
		//	printf("numero di pallette che entrano %d \n", count);
			...
			correct<<<count0/1024,1024,0,stream0>>>(dev_stato_0,dev_ids_0,count0);
			correct<<<count1/1024,1024,0,stream1>>>(dev_stato_1,dev_ids_1,count1); 					
