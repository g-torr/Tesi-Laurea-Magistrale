...
struct points{	float* x;
		float* y;};
struct point{	float x;
		float y;};

struct configurazione{	points	eta;
			points	r;
			points	forza;
			bool* 	inside;};
...
void	stampa(configurazione stato, int i){int id;FILE*f;FILE*g;char indirizzo_posizione[50];char indirizzo_forza[50];
		...
		for(id=0;id<particelle;id++){fprintf(f,"%f		%f \n",stato.r.x[id],stato.r.y[id]); //printf("sto scrivendo %d\n",id);
					fprintf(g,"%f		%f \n",stato.forza.x[id],stato.forza.y[id]);}
					fclose(f);fclose(g);}
...
__global__ void	inizializza(configurazione stato,curandState* gen_random){point temp;
							...
							temp.y=-half_size+size*curand_uniform(&gen_random[id]);}
								...
								while(is_in(temp)); 		 	     							stato.r.x[id]=temp.x;
						stato.r.y[id]=temp.y;
						stato.eta.x[id]=0.f;	
						stato.eta.y[id]=0.f;
...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
			point r,v;
			int id=threadIdx.x+blockIdx.x*blockDim.x; 	
			v.x=stato.eta.x[id];v.y=stato.eta.y[id];	r.x=stato.r.x[id];	r.y=stato.r.y[id];
			...
			//if((r.x>ottimizza[0].x)&&(r.x<ottimizza[1].x)&&(r.y<ottimizza[2].x)&&(r.y>ottimizza[0].y)){		
					...
					else {stato.inside[id]=false;stato.forza.x[id]=0.f; stato.forza.y[id]=0.f;	
			...
			stato.eta.x[id]=v.x;	stato.eta.y[id]=v.y;	stato.r.x[id]=r.x;	stato.r.y[id]=r.y;	
				/*	if (in){
										f=r;
										r=segmento_vicino(r);
...
__global__ void correct(configurazione stato,curandState* gen_random,int * ids,int count){
				...
				int id=ids[k]; 	
				r.x=stato.r.x[id];	r.y=stato.r.y[id];
				f=r;
				r=segmento_vicino(r);
				f.x=(f.x-r.x)/(dt*mobility);
				f.y=(f.y-r.y)/(dt*mobility); 
				stato.forza.x[id]=f.x;	stato.forza.y[id]=f.y;
				stato.r.x[id]=r.x;	stato.r.y[id]=r.y;		
...
cudaMalloc((float**)&dev_stato.eta.x,particelle*sizeof(float));
cudaMalloc((float**)&dev_stato.eta.y,particelle*sizeof(float));
cudaMalloc((float**)&dev_stato.r.x,particelle*sizeof(float));
cudaMalloc((float**)&dev_stato.r.y,particelle*sizeof(float));
cudaMalloc((float**)&dev_stato.forza.x,particelle*sizeof(float));
cudaMalloc((float**)&dev_stato.forza.y,particelle*sizeof(float));
...
stato.eta.x=(float*)malloc(particelle*sizeof(float));
stato.eta.y=(float*)malloc(particelle*sizeof(float));
stato.r.x  =(float*)malloc(particelle*sizeof(float));
stato.r.y  =(float*)malloc(particelle*sizeof(float));
stato.forza.x=(float*)calloc(particelle,sizeof(float));
stato.forza.y=(float*)calloc(particelle,sizeof(float));
...
for(t=0;t<numero_passi;t++){
		...
		//	printf("numero di pallette che entrano %d \n", count);
			...
			cudaMemcpy(stato.r.x,dev_stato.r.x,particelle*sizeof(float),cudaMemcpyDeviceToHost);
			cudaMemcpy(stato.r.y,dev_stato.r.y,particelle*sizeof(float),cudaMemcpyDeviceToHost);
			cudaMemcpy(stato.forza.x,dev_stato.forza.x,particelle*sizeof(float),cudaMemcpyDeviceToHost);
			cudaMemcpy(stato.forza.y,dev_stato.forza.y,particelle*sizeof(float),cudaMemcpyDeviceToHost);
				stampa(stato,i);i++;
...
//cudaDeviceSynchronize();
			...
			cudaMemcpy(stato.forza.y,dev_stato.forza.y,particelle*sizeof(float),cudaMemcpyDeviceToHost);
stampa(stato,i);
cudaFree(dev_stato.r.x);
cudaFree(dev_stato.eta.x);
cudaFree(dev_stato.forza.x);
free(stato.r.x);
free(stato.forza.x);
free(stato.eta.x);
