...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
			point r,v,f;
			int id=threadIdx.x+blockIdx.x*blockDim.x; 
			//curandState temp_state=gen_random[id]; questo tipo di soluzione, ovvero copiare il generatore nella variabile locale mi da inaspettatamente untempo di esecuzione più alto		
...
for(t=0;t<numero_passi;t++){
		...
		//	printf("numero di pallette che entrano %d \n", count);
			...
			correct<<<count/512,512>>>(dev_stato,dev_ids,count);					
