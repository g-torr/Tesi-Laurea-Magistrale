...
__global__ void correct(configurazione stato,point* posizioni,int * ids,int count){
				point r,f;
...
point* posizioni;
cudaMalloc((point**)&posizioni,particelle*sizeof(point));
...
for(t=0;t<100;t++){
			...
			for(int j=0;j<particelle;j++){
					if(stato.inside[j]==true){ 
							ids[count]=j; cudaMemcpy(posizioni+count,dev_stato.r+j,sizeof(point), cudaMemcpyDeviceToDevice); 
count++;}
							}
//			printf("numero di pallette che entrano %d \n", count);
			cudaMemcpy(dev_ids,ids,count*sizeof(int),cudaMemcpyHostToDevice);
			correct<<<count/512,512>>>(stato,posizioni,dev_ids,count);					
