...
__global__ void	evolvi(configurazione stato,curandState* gen_random,int passi_salvataggio){	
						 point r,v,f;
						int id=threadIdx.x+blockIdx.x*blockDim.x;
						v=stato.eta[id];	r=stato.r[id]; 	
						for(int t=0;t<passi_salvataggio;t++){
				...
				//if((stato.r[id].x>ottimizza[0].x)&&(stato.r[id].x<ottimizza[1].x)&&(stato.r[id].y<ottimizza[2].x)&&(stato.r[id].y>ottimizza[0].y)) {		
					...
					if (!is_in(r)){
						...
						stato.r[id]=r; stato.eta[id]=v;		stato.forza[id]=f;
...
curandState * generatori_random_0;//alloco il generatore dei numeri random sul device
curandState * generatori_random_1;
if (numero_passi> pow(2,67)) printf("ATTENZIONE! ricontrollare il generatore di numeri casuali"); 
cudaMalloc((void**)&generatori_random_0,particelle/2*sizeof(curandState));
cudaMalloc((void**)&generatori_random_1,particelle/2*sizeof(curandState));
setup_random_kernel<<<blocks,threads>>>(generatori_random_0,0);
setup_random_kernel<<<blocks,threads>>>(generatori_random_1,particelle/2);


inizializza<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random_0);
inizializza<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random_1);

evolvi<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random_0,passi_salvataggio);
evolvi<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random_1,passi_salvataggio);
