...
def	forza(i,fmax,nb):
	...
	h,b = histogram(modulo, bins=linspace(-.1,fmax,nb),normed=True)
	plot(b[:-1],h)	
	return h	
...
def	media_temporale(nb):
	...
	xlabel("modulo della forza")
	ylabel("probability density")
	D=0.1	
	tau=0.06
	nf=8 #numero di file letti
	fmax=4*sqrt(D/tau)
...
def	campo_vettoriale(i=8,size=3.):
	...
	modulo=sqrt(fx**2+fy**2)
	h,bx,by = histogram2d(x,y,bins=arange(-size/2.,size/2,.05))
	hfx,bx,by = histogram2d(x,y,bins=arange(-size/2.,size/2.,.05),weights=fx)
	hfy,bx,by = histogram2d(x,y,bins=arange(-size/2,size/2,.05),weights=fy)
	hfx = where(h>0,hfx/h,0)
	hfy = where(h>0,hfy/h,0)
	f=hfx**2+hfy**2
	quiver(by[:-1]+.1,bx[:-1],hfy,hfx,f,scale=20)
