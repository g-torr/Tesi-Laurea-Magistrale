...
__device__ point segmento_vicino(point P){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
    ...
    if(t<0.0f){t=(float)0.0;}
    if(t>1.0f){t=(float)1.0;} 
...
__global__ void	inizializza(configurazione stato,curandState* gen_random){point temp;
						...
						stato.r[id].y=temp.y;
						stato.eta[id].x=(float)0;	
						stato.eta[id].y=(float)0;
...
inizializza<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random_1);
			...
			evolvi<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random_0);
			evolvi<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random_1);
for(i=0;i<9;i++){
//for(t=0;t<10;t++){
			evolvi<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random_0);
			evolvi<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random_1);
