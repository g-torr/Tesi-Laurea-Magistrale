...
__global__ void	evolvi(configurazione stato,curandState* gen_random,int k){	
						 point r,v,f;
						int id=threadIdx.x+blockIdx.x*blockDim.x; 	
						int i=k*blockDim.x*gridDim.x+id;
						...
						v.x=v.x-v.x*costanti[0]+curand_normal(&gen_random[i])*costanti[1];   	
						v.y=v.y-v.y*costanti[0]+curand_normal(&gen_random[i])*costanti[1]; 	
						r.x=r.x+v.x*dt;
						r.y=r.y+v.y*dt;	
						f.x=0.f; f.y=0.f;
...
//for(t=0;t<10;t++){
			evolvi<<<blocks/4,threads,0,stream0>>>(dev_stato_0,generatori_random,0);
			evolvi<<<blocks/4,threads,0,stream1>>>(dev_stato_1,generatori_random,1);
			evolvi<<<blocks/4,threads,0,stream2>>>(dev_stato_2,generatori_random,2);
			evolvi<<<blocks/4,threads,0,stream3>>>(dev_stato_3,generatori_random,3);
