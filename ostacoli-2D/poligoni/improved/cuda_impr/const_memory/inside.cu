...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
						...
						int id=threadIdx.x+blockIdx.x*blockDim.x; 	
						stato.eta[id].x=stato.eta[id].x-(1.f/tau)*stato.eta[id].x*dt+sqrt(D)*curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau;      			
						stato.eta[id].y=stato.eta[id].y-(1.f/tau)*stato.eta[id].y*dt+sqrt(D)*curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau;			
