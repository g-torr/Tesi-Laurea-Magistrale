...
__global__ void segmento_vicino(point P,point* closest,int id){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
	int i=0;float t;float d; 	
	 __shared__ point p_parziale[N];
	__shared__ float d_parziale[N];
			int cacheindex=threadIdx.x;

	t = ((P.x-vertice[cacheindex].x)*(vertice[cacheindex+1].x-vertice[cacheindex].x)+(P.y-vertice[cacheindex].y)*(vertice[cacheindex+1].y-vertice[cacheindex].y))/
    ...
    p_parziale[cacheindex].x = vertice[cacheindex].x + (vertice[cacheindex+1].x-vertice[cacheindex].x)*t; 
    p_parziale[cacheindex].y = vertice[cacheindex].y + (vertice[cacheindex+1].y-vertice[cacheindex].y)*t;  
    d_parziale[cacheindex]=pow((P.x- p_parziale[cacheindex].x),2)+pow(P.y-p_parziale[cacheindex].y,2);
		__syncthreads();
	
	 
	
	d=d_parziale[0]; int j=0;
	for(int i=1;i<N;i++){
			if (d_parziale[i]<d) {d=d_parziale[i];j=i;}
				}
	 closest[id]=p_parziale[j];
