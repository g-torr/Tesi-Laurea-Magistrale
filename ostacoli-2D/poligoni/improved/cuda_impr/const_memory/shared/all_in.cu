...
__device__ point segmento_vicino(point P,point* geo){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
	int i=0;float t;float d; 
	 point closest;
	t = ((P.x-geo[0].x)*(geo[0+1].x-geo[0].x)+(P.y-geo[0].y)*(geo[0+1].y-geo[0].y))/
								((geo[0+1].x-geo[0].x)*(geo[0+1].x-geo[0].x)+(geo[0+1].y-geo[0].y)*(geo[0+1].y-geo[0].y));

	 if(t<0.0f){t=0.0f;}
 	 if(t>1.0f){t=1.0f;} 


    closest.x = geo[0].x+ (geo[0+1].x-geo[0].x)*t; 
    closest.y = geo[0].y+ (geo[0+1].y-geo[0].y)*t;  
	d=(P.x- closest.x)*(P.x- closest.x)+(P.y- closest.y)*(P.y- closest.y);

	point temp;
	float d_temp;
 	for(i=1;i<N;i++){
    t = ((P.x-geo[i].x)*(geo[i+1].x-geo[i].x)+(P.y-geo[i].y)*(geo[i+1].y-geo[i].y))/
								((geo[i+1].x-geo[i].x)*(geo[i+1].x-geo[i].x)+(geo[i+1].y-geo[i].y)*(geo[i+1].y-geo[i].y));
    ...
    temp.x = geo[i].x+ (geo[i+1].x-geo[i].x)*t; 
    temp.y =geo[i].y+ (geo[i+1].y-geo[i].y)*t;  
...
__device__ int wn_PnPoly( point P,point* geo, int n )
{
    int    wn = 0;    // the  winding number counter

    // loop through all edges of the polygon
    for (int i=0; i<n; i++) {   // edge from V[i] to  V[i+1]
        if (geo[i].y <= P.y) {          // start y <= P.y
            if (geo[i+1].y  > P.y)      // an upward crossing
                 if (isLeft( geo[i], geo[i+1], P) > 0)  // P left of  edge
                     ++wn;            // have  a valid up intersect
        }
        else {                        // start y > P.y (no test needed)
            if (geo[i+1].y  <= P.y)     // a downward crossing
                 if (isLeft( geo[i], geo[i+1], P) < 0)  // P right of  edge
...
__device__ bool is_in(point P,point* geo){int wn=wn_PnPoly(P,geo,N);
					//printf("il wn=%d \n",wn);
					if (wn>0) return true;
							else return false;
						}
...
__device__ void	inizializza(point* r,point* v,curandState* gen_random){point temp;
							int id=threadIdx.x+blockIdx.x*blockDim.x;	
						
							do {
							temp.x=-half_size+size*(float)curand_uniform(&gen_random[id]);
							temp.y=-half_size+size*(float)curand_uniform(&gen_random[id]);}
							
								while(is_in(temp,vertice)); 		 	     							(*r).x=temp.x;
						(*r).y=temp.y;
						(*v).x=0.f;	
						(*v).y=0.f;
...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
						 point r,v,f;int passi_salvataggio =(int)(tsalva/dt);
						int id=threadIdx.x+blockIdx.x*blockDim.x; 	
						inizializza(&r,&v,gen_random);
						__shared__ point geo[N+1];
				for(int i=0;i<N+1;i++){geo[i]=vertice[i];} 
				for(int t=0;t<passi_salvataggio;t++){
						v.x=v.x-(1/tau)*v.x*dt+sqrt(D)*(float)curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau;   	
						v.y=v.y-(1/tau)*v.y*dt+sqrt(D)*(float)curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau; 	
						r.x=r.x+v.x*dt;
						r.y=r.y+v.y*dt;	
						f.x=0.f; f.y=0.f;
