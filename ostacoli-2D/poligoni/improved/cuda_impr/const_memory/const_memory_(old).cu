...
#define dt 0.01
#define mobility 1
const int durata=size*size*20/D;
...
struct configurazione{	point*	eta;
			point*	r;
			point*	forza;};
__constant__ point vertice[4+1];
...
__device__ void segmento_vicino(int N, point P,point* closest,int id){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
...
__device__ bool is_in(point P,int N){int wn=wn_PnPoly(P,N);
					//printf("il wn=%d \n",wn);
					if (wn>0) return true;
							else return false;
						}
...
__global__ void	inizializza(configurazione stato,int N,curandState* gen_random){point temp;
							int id=threadIdx.x+blockIdx.x*blockDim.x;	
							...
							temp.y=-half_size+size*curand_uniform(&gen_random[id]);}
								...
								while(is_in(temp,N)); 		 	     							stato.r[id].x=temp.x;
						stato.r[id].y=temp.y;
						stato.eta[id].x=0;	
						stato.eta[id].y=0;
...
__global__ void	evolvi(configurazione stato,point* vertice, int N,geometria ottimizza,curandState* gen_random){	
				...
				if((stato.r[id].x>ottimizza.vertici[0].x)&&(stato.r[id].x<ottimizza.vertici[1].x)&&(stato.r[id].y<ottimizza.vertici[2].x)&&(stato.r[id].y>ottimizza.vertici[0].y)) {		
					if (is_in(stato.r[id],N)){
										stato.forza[id]=stato.r[id];
										segmento_vicino(N,stato.r[id],stato.r,id);
...
cudaMemcpyToSymbol(vertice,scheletro.vertici,(N+1)*sizeof(point));
...
inizializza<<<blocks,threads>>>(dev_stato,N,generatori_random);
...
for(t=0;t<numero_passi;t++){
			evolvi<<<blocks,threads>>>(dev_stato,vertice,N,dev_ottimizza,generatori_random);
