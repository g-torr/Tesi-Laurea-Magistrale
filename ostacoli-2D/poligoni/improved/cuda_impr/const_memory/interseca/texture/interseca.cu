...
texture<point> text_r;
texture<float> text_prova; 
...
__device__ bool interseca(double*t, double*T,point P_old,point P_nuovo,point V1,point V2){double x1,x2,X1,X2,y1,y2,Y1,Y2;
					...
					*t=(X2*(y1 - Y1) + x1*(Y1 - Y2) + X1*(-y1 + Y2))/(-((X1 - X2)*(y1 - y2)) + (x1 - x2)*(Y1 - Y2));					
					if((*t>=0.0)&&(*t<=1.0)&&(*T>-0.000000000001)&&(*T<=1.)) return true;
     						else return false;	}
...
__global__ void	evolvi(configurazione stato,curandState* gen_random){	
						...
						stato.eta[id].y=stato.eta[id].y-(1/tau)*stato.eta[id].y*dt+sqrt(D)*curand_normal(&gen_random[id])*sqrt(2.)*sqrt(dt)/tau;			point punto;punto=tex1Dfetch(text_r,id);punto.y=tex1Dfetch(text_prova,2);
						proposta.x=punto.x+stato.eta[id].x*dt;
						proposta.y=punto.y+stato.eta[id].y*dt;	
...
cudaBindTexture(NULL,text_r,stato.r,particelle*sizeof(point));
float prova[4];
cudaBindTexture(NULL,text_prova,prova,4*sizeof(float));
