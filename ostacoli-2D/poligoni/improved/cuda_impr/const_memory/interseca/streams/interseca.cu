...
__device__ inline double	isLeft( point P0, point P1, point P2 ){
...
__device__ bool interseca(double*t, double*T,point P_old,point P_nuovo,point V1,point V2){double x1,x2,X1,X2,y1,y2,Y1,Y2;
...
__device__ bool closest(point P_old,point P, point* nuovo){double t,temp_t,T,temp_T;int i;bool flag=false;
				T=1;				
				for(i=0;i<N;i++){ 
...
void alloco_punti(point* scheletro){  int i=0; float a,b;
					...
					for(i=0;i<N;i++){
							fscanf(f,"%lf %lf",&scheletro[i].x,&scheletro[i].y);
							}
					fclose(f);
...
inizializza<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random);
inizializza<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random);
i=0;
for(t=0;t<numero_passi;t++){
//for(t=0;t<10;t++){
			evolvi<<<blocks/2,threads,0,stream0>>>(dev_stato_0,generatori_random);
			evolvi<<<blocks/2,threads,0,stream1>>>(dev_stato_1,generatori_random);
	   if((t% passi_salvataggio==0)&&(t>0)){printf("siamo a %d/9 \n",i);
			...
			cudaMemcpyAsync(stato.forza+(particelle/2),dev_stato_1.forza,(particelle/2)*sizeof(point),cudaMemcpyDeviceToHost, stream1);  		cudaDeviceSynchronize();
			stampa(stato,i);i++;}
