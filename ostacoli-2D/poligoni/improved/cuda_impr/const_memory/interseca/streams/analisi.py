...
def	campo_vettoriale(i=8):
	...
	'''for j in arange(0,len(modulo)):	
		if (modulo[j]!=0):
			x=append(x[:j],x[j+1:])
			y=append(y[:j],y[j+1:])
			fx=append(fx[:j],fx[j+1:])
			fy=append(fy[:j],fy[j+1:])
	modulo=modulo[modulo!=0]
	fx=fx/modulo
	fy=fy/modulo'''
	#streamplot(x,y,fx,fy,color=x,linewidth=2, cmap=plt.cm.autumn)	
	quiver(x,y,fx,fy,modulo, cmap=cm.seismic,units='inches',scale=2**-.5)
	colorbar()
