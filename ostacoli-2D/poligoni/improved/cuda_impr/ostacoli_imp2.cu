...
__global__ void segmento_vicino(point * V, int N, point P,point* closest){ //P è il punto rispetto al quale viene cercato il segmento più vicino,  la funzione restituisce closest, che è il punto sul perimetro del poligono più vicino a P, d è tale distanza minima.
...
/*__global__ void	evolvi(configurazione stato,geometria scheletro, int N,geometria ottimizza,curandState* gen_random){point closest;	
						int id=threadIdx.x+blockIdx.x*blockDim.x; 	
				...
				if((stato.r[id].x>ottimizza.vertici[0].x)&&(stato.r[id].x<ottimizza.vertici[1].x)&&(stato.r[id].y<ottimizza.vertici[2].x)&&(stato.r[id].y>ottimizza.vertici[0].y)) {		
					if (is_in(scheletro,stato.r[id],N)){
										...
										stato.r[id]=segmento_vicino(scheletro.vertici,N,stato.r[id],&closest);
...
configurazione dev_stato;//alloco lo stato del sistema
...
double time_sec = 
 ...
 point a,b; a.x=1;a.y=3.;
segmento_vicino<<<1,2>>>(dev_scheletro.vertici,N,a,&b);
printf("%f %f ",b.x,b.y);
